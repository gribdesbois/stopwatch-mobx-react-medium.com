...
import { Timer, TimerStore } from './stores/TimerStore'
import ILap from './@types/ILap'

const TimerDisplay = ({ timer, text }: { timer: Timer; text: string }) => (
  <div
    style={{
...
const Main = observer(({ timerStore }: { timerStore: TimerStore }) => {
  let firstButton
  let secondButton
  ...
  if (!timerStore.isRunning) {
    ...
    firstButton = (
      <button style={ButtonStyle} onClick={() => timerStore.resetTimer()}>
        reset
      </button>
    )
      ...
      <div>
        ...
        <div>
          {timerStore.lapData.map((el) => (
            <TimerDisplay key={el.lap.id} timer={el.lap} text={el.text} />
          ))}
        </div>
      </div>
    </div>
